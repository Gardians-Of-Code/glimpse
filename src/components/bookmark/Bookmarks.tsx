...
const urlRegex = new RegExp(/^(http?|https?|ftp):\/\//g);
...
function Bookmarks() {
  ...
  //all ml tags
  const [mlTags, setMlTags] = useState<TagType[]>(
  ...
  const handleAddBookmark = async (e: React.FormEvent) => {
    ...
    if (currUrl) {
      ...
      tagsToAdd.forEach((tag, index) => {
        tagsToAdd[index] = tag.trim();
      });
      //remove repeated elements from tagsToAdd
      const uniqueTagsArray = tagsToAdd.filter(
        (item, index) => tagsToAdd.indexOf(item) === index
      );
      ...
      // const mlTags = await getMLTags.json();
      // console.log("mlTags", mlTags.tags);
  ...
  const updateTags = (oldtags: string, newtags: string) => {
    ...
    oldTagsArray.forEach((tag, index) => {
      oldTagsArray[index] = tag.trim();
    });
    ...
    newTagsArray.forEach((tag, index) => {
      newTagsArray[index] = tag.trim();
    });
    console.log("newTagsArray", newTagsArray);

    //remove repeated tags from newTagsArray
    const uniqueTagsArray = newTagsArray.filter(
      (item, index) => newTagsArray.indexOf(item) === index
    );
    // Determine tags that were added, removed, or retained
    const tagsAdded = uniqueTagsArray.filter(
      (tag) => !oldTagsArray.includes(tag)
    );
    const tagsRemoved = oldTagsArray.filter(
      (tag) => !uniqueTagsArray.includes(tag)
    );
    const tagsRetained = uniqueTagsArray.filter((tag) =>
      oldTagsArray.includes(tag)
    );

    // Update custom tags
    const updatedCustomTags: TagType[] = [];
